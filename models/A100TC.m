function D = A100TC(alpha, A, B, beta, C, informat, outformat)
%
% A100TC  Compute GEMM with a model of a tensor core of the A100 GPU.
%
% This function evaluates the expression D = A * B + C using the 
% A100 TC numerical-feature-based model. The accumulation of block
...
allowedOutFormats = {'fp32', 'single', 'binary32',...
    'fp16', 'binary16', 'half'};
allowedInFormats = {'fp16','binary16', 'half',...
    'bf16','bfloat16','tensorfloat32','tf32'};
...
if ismember(informat, {'fp16','half','binary16'})
    if exist('outformat', 'var')
        if ismember(outformat, {'fp16','binary16','half'})
            ...
            def_params.min_exp_limit   = -20; % Minimum exponent allowed for product alignment
        end  
    end
