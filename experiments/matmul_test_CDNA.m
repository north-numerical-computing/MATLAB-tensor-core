...
function C = matmul(A, B, model, p, u, inopt, outopt)
  ...
  for j=1:p
      for k=1:p
          if (j+k-2 < p)
              if strcmp(model, 'mi100')
                  C = MI100MC(u^(j+k-2), A{j}, B{k}, 1, C,...
                      inopt.format);
              elseif strcmp(model, 'mi250')
                  C = MI250MC(u^(j+k-2), A{j}, B{k}, 1, C,...
                  inopt.format);
              elseif strcmp(model, 'mi300x')
                  C = MI300XMC(u^(j+k-2), A{j}, B{k}, 1, C, ...
                      inopt.format);
              end
          end
      end
